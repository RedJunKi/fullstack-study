...
MyReservationInitializer.prototype = {
    loadData : function() {
        ...
        httpRequest.open('GET', "/api/reservations?reservationEmail=" + userEmail);
        httpRequest.send();
    }
}
...
ReservationCardsContainer.prototype = {
    ...
    submitCancel : function(reservationId) {
        let httpRequest = new XMLHttpRequest();
        httpRequest.onreadystatechange = function() {
            if (httpRequest.readyStatus === XMLHttpRequest.DONE && httpRequest.status !== 200) {
                alert("요청을 처리할 수 없습니다.");
            }
        }

        httpRequest.open('PUT', "/api/reservations/" + reservationId);
        httpRequest.send();
    },
    ...
    initReviewButton : function() {
        ...
        reviewButtons.forEach(function(item) {
            item.addEventListener("click", function(evt) {
                ...
                location.href = "/reviewwrite?reservationInfoId=" + reservationId;
            });
        });
    }
}