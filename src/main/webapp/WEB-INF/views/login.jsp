   ...
   <body class="biz  ko" >
                  ...
                  <div >
                     <form name="confirm_form" class="ng-pristine ng-valid" id="form1" action="./login" method="post">
                        ...
                        <div class="login_form">
                           ...
                           <input type="text" class="login_input ng-pristine ng-untouched ng-valid ng-empty" id="resrv_id" name="resrv_email" aria-invalid="false" placeholder="예약자 이메일 입력" title="예매자이메일">
                           <ul id="error_list"></ul>
                           </div>
      ...
      <script>
            ...
            btn.addEventListener("click", function(evt) {
                evt.preventDefault();
                let emailValid = document.querySelector("[name=resrv_email]").value;
                let bValid = (/\w+@\w+\.\w+/).test(emailValid);
                let parent = document.querySelector("#form1");

                let errorList = document.getElementById("error_list");
                errorList.innerHTML = "";
                if (!bValid) {
                    let child = document.createElement("li");
                    let validMessage = '이메일 형식이 유효하지 않습니다.<br>예) example@naver.com';
                    child.innerHTML = validMessage;
